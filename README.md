...
## 最新论文与按词定位卡片（2026-09-25）

- [论文修订稿 PDF](results/paper_manuscript_v2_20260925/多模态情感预测_论文修订稿.pdf) / [可编辑 Word](results/paper_manuscript_v2_20260925/多模态情感预测_论文修订稿.docx)：图4已换成按词语估计时间选帧的卡片。
- [正文源文件](results/paper_manuscript_v2_20260925/论文正文.md) / [本轮修订说明](results/paper_manuscript_v2_20260925/本轮修订说明.md)。当前为正文修订稿，未套用当届官方提交模板。
- [卡片说明](results/q1_word_review_20260925/评价口径与卡片说明.md)：图片、词区间、声音和人脸状态用编号及连线对应；不把覆盖率解释为真实对齐准确率。
- 查看与重建步骤统一在 [运行使用说明](运行使用说明.md) 的最新章节。原始视频和训练权重仍需另行准备。

本项目为 2026 年第二十三届中国研究生数学建模竞赛 E 题（复杂场景下多模态情感预测的数学建模与算法设计）的工程实现代码库。

> GitHub 仓库包含代码、配置、说明、问题一派生特征，以及问题二的论文写作资料和关键实验结果。问题一旧版和待人工审查的新版派生特征（`data/processed/q1/`、`data/processed/q1_v2/`）各 100 份。`data/raw/` 原始附件、其他处理数据、`models/` 预训练权重、`vendor/` 本机安装包、`checkpoints/` 和运行日志不上传。下列 `D:/...` 路径是原开发机示例，克隆后需按本机位置调整；没有原始附件和模型文件时，特征提取命令不能直接运行。问题一提取的特征不用于问题二、三训练。
...
**写第二问论文的队友从这里开始**：[问题二_论文写作资料与实验全过程.md](问题二_论文写作资料与实验全过程.md)。它按题意、数学模型、完整实验顺序、实测结果、图表口径和不能越界的结论整理；对应的关键 CSV、训练摘要、附件三 30 条预测及源码随仓库提供。当前主模型为 **C_uni**：三模态分支 Transformer、直接拼接、单模态辅助损失权重 **0.1**、学习率 `3e-4`，不含门控。版本以 [configs/q2_final.json](configs/q2_final.json) 为准；最终[附件三预测](results/q2_attachment3_candidate_q2_c_aug_1layer_concat_eff_tonight_uni_20260924/predictions.csv)已更新。历史 selection.json 和实验报告保留各阶段结论。原始附件、冻结 BERT 和训练权重依工程规则不进入 Git，因此仓库提供的是论文与代码复核材料，不是可单靠克隆仓库从零训练的完整数据包。

**问题一视觉改进候选（2026-09-23）**：原始结果保留在 `data/processed/q1/`；裁剪恢复人脸的候选结果在 `data/processed/q1_v2/`。对照指标和人工审查顺序见 [results/q1_v2/人工审查指引.md](results/q1_v2/人工审查指引.md)。新版需要人工确认目标说话人，当前不视为正式通过。

新版的原视频画面审查图和人脸框拼图已在本机生成，但因包含原视频人物画面，不随本次 Git 更新上传；持有题目原视频的队友可按 `运行使用说明.md` 第 8 节重画。
