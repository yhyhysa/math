...
**写第二问论文的队友从这里开始**：[问题二_论文写作资料与实验全过程.md](问题二_论文写作资料与实验全过程.md)。它按题意、数学模型、完整实验顺序、实测结果、图表口径和不能越界的结论整理；对应的关键 CSV、训练摘要、附件三 30 条预测及源码随仓库提供。当前三模态方案使用分模态时序 Transformer、直接拼接、学习率 `3e-4`。原始附件、冻结 BERT 和训练权重依工程规则不进入 Git，因此仓库提供的是论文与代码复核材料，不是可单靠克隆仓库从零训练的完整数据包。
