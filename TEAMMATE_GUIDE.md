...
| `data/processed/q1_v2/` | 100 条视觉改进候选的 JSON/NPZ，共 200 个文件；保留旧版供对照，仍待人工审查。 |
...
`data/raw/` 及其他原始附件、`models/` 预训练权重、`vendor/` 本机依赖、`checkpoints/`、`submission/`、虚拟环境、缓存和大日志均由 `.gitignore` 排除。尤其不要为了让代码“能跑”而把原始视频、附件二至四或模型权重直接提交到 Git。旧版与新版 Q1 派生特征各约 18 MB；整库工作树中的这些外部数据和模型不在仓库里。
...
2026-09-23 新增 `q1_v2` 视觉改进候选：只对旧版人脸覆盖率低于 50% 的视频重新进行居中裁剪和视觉提取，文字、音频与词时间保持不变。新版在 `data/processed/q1_v2/`，比较表和人工审查材料在 `results/q1_v2/`；目前仍需核对裁剪后检到的人脸是否为目标说话人。完整重跑方法和指标口径在 [运行使用说明.md](运行使用说明.md) 第 8 节。

新版原始画面缩略图与人脸框拼图保留在审查者本机，未随本次 Git 更新上传；队友需要另有题目原视频，才能重画这些图。新版特征、指标表和审查说明可以直接从仓库获取。
